...
async def denied(self, ctx, avatar):
    x = discord.Embed(
        title="Shop",
        color=0xFF0000,
        description=f"Sorry {ctx.author.mention}, I can't give credit! Come back when you're a little... mmm... richer",
    )
    x.set_author(name=ctx.author.name, icon_url=avatar)
    x.set_image(
        url="https://cdn.discordapp.com/attachments/821123717223415809/823315107261055006/morshu.gif"
    )
...
async def mining(self, ctx, num1, userid):
    ...
    if x <= num2:
        ...
        await ctx.send(
            f"Woah, you found a gold ore! It's worth {y} :coin:! Lucky you! {memb.mention}"
        )
...
class Economy(commands.Cog):
    ...
    @commands.command(
        brief="Displays the user's balance",
    ...
    async def balance(self, ctx, user: discord.Member = None):
        if user == None:
            ...
            balance = getUserMoney(self, userid)
            await ctx.send(
                f"{ctx.author.mention}, {user.display_name}'s balance is {balance} :coin:"
            )

    # SHOPPING
    @commands.command(
        brief="Access the shop",
        description="Main command to access, buy and find items in the shop.",
    )
    async def shop(self, ctx, arg="help", *, item=""):
        ...
        if "list" in arg:
            shop = discord.Embed(
                title="Shop",
                description="The one destination for all your needs!",
                color=0xFF0000,
            )
            ...
            for i in range(
                len(pickaxetiers)
            ):  # USING THIS INSTEAD OF MANUALLY PUTTING IT ALL OUT
                if i + 1 <= pickaxelevel:
                    output = (
                        output
                        + f"{pickaxetiers[i]} pickaxe: You already own this item \n"
                    )
                else:
                    output = (
                        output
                        + f"{pickaxetiers[i]} pickaxe: {pickaxecost[pickaxetiers[i]]} :coin:\n"
                    )
        ...
        # POINTING USERS TO THE RIGHT COMMANDS
            ...
            await ctx.send(
                "``shop buy [item]`` to buy things and ``shop list`` to see list."
            )
        # BUYING PICKAXES
        elif "buy" in arg:
            pickaxelevel = getPickaxeLevel(self, userid)
            ...
            for i in item.lower().split():
                if i in items:  # CHECKING THE PRICE
                    ...
                    if money < cost:
                    ...
                    ):  # DOESNT REALLY MAKE SENSE TO BUY SOMETHING YOU HAVE
                        ...
                        await ctx.send(
                            f"You have successfully gotten a {i} pickaxe! Happy mining!"
                        )
        else:
            await ctx.send(
                "Invalid command. ``shop buy [item]`` to buy things and ``shop list`` to see list."
            )

    # MINING COMMAND
    @commands.cooldown(rate=1, per=5, type=commands.BucketType.user)
    @commands.command(
        brief="Mines for money",
        description="Randomized chance to find ores or scraps. Better pickaxes give out more often rewards",
    )
    async def mine(self, ctx):
        ...
        if pickaxelevel == 0:
            await ctx.send(
                f"You don't own a pickaxe yet! You need to buy it from the shop! Check our collection at ```^shop list```"
            )
    ...
    # LEADERBOARD SYSTEM
    @commands.command(
        brief="Leaderboard of the richest users.",
        description="This is a leaderboard of the richest users on the server.",
    )
    async def leaderboard(self, ctx):
        ...
        embed.set_author(
            name="Top users",
            icon_url="https://cdn.discordapp.com/avatars/820839140111155200/9453547c9ae893eb3a3833ff7734a4ac.webp?size=1024",
        )
        for k in richest[:5]:  # <-- change to 10 if you want top 10
            user = self.bot.get_user(leaderboard[k]["userid"])
            money = leaderboard[k]["money"]
            embed.add_field(
                name=user.display_name, value=f"{money} :coin:", inline=False
            )
    ...
    @commands.command(brief="The house always wins, you know.", description="Slot machine.")
        ...
        if amount > money:
            ...
            if amount <= 0:
                ...
                if rand > 80:
                    ...
                    pot = amount * 3
                    await ctx.send(
                        f"{ctx.message.author} has won {pot} coins!!! Hurrah!"
                    )
