...
class Fun(commands.Cog):
    ...
    @commands.command(
        brief="Wooks up a summawy of a wikipedia page",
        description="Wooks up a summawy of a wikipedia page",
    )
    async def wikowo(self, ctx, *, page):
        ...
        try:
            ...
            summary_list = textwrap.wrap(
                owo, 2000, break_long_words=False
            )  # THIS ONE IS TO BREAK UP THE MESSAGE INCASE IF ITS TOO LONG
            for i in summary_list:
                await ctx.channel.send(i)
            await ctx.channel.send(
                f"<{site.url}>"
            )  # SENDS THE URL SO USERS CAN INVESTIGATE FURTHER
    ...
    @commands.command(brief="OwO, whats this? uwaaa >.<")  # LITERALLY JUST OWOFIES THE TEXT
    ...
    @commands.command(
        ...
        description="For some reason, people always lose.",
    )
    async def rps(self, ctx, choice=""):
        if choice == "":
            await ctx.send(
                "You need to pick between ``rock``, ``paper`` or ``scissor``"
            )
    ...
    @commands.command(
        brief="Why don't you try me out?", description="we love some puns dont we"
    )
    async def complementarybread(self, ctx):
        ...
        await user.send(
            "https://cdn.discordapp.com/attachments/821123717223415809/827637301953429514/complementarybread.png"
        )
        print(f"Welcome to the bread bank, {user}")

    # POST HUMOR FUCKING SUCKS
    # LITERALLY JUST RETURNS "SUS"
    @commands.command(brief="thats a bit SUSSY", description="amogus")
    ...
    @commands.command(brief="moo", description="moo")
    ...
    @commands.command(brief="Hug a user!", description="Hug a user!")
    async def hug(self, ctx, arg: discord.Member = None):
        await ctx.send(
            f"<@{ctx.message.author.id}> is hugging <@{arg.id}>! :people_hugging:"
        )

    # WE'RE EXTREMELY MATURE
    # THIS ONE JUST SENDS A "UR MAMA" JOKE
    @commands.command(
        brief="heehoo i have the mentality of a six year old",
        description="im still six years old",
    )
